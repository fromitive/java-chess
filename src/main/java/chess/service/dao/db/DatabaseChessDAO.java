...
public class DatabaseChessDAO implements ChessDAO {
    ...
    public DatabaseChessDAO(Connection connection) {
    ...
    private void validateEmpty(ResultSet resultSet) throws SQLException {
        if (!resultSet.isBeforeFirst()) {
            throw new RuntimeException("DB에 기존 게임에 대한 정보가 존재하지 않습니다.");
        }
    }

    private Map<Position, Piece> buildBoard(ResultSet resultSet) throws SQLException {
    ...
    public void updateBoard(Board board) {
        try (var statement = connection.prepareStatement(INSERT_BOARD_QUERY)) {
            ...
            board.getBoard().entrySet().stream()
                    .filter(entry -> entry.getValue() != Piece.EMPTY_PIECE)
    ...
    private void addPieceIntoStatement(Entry<Position, Piece> entry, PreparedStatement statement) {
    ...
    private void setInsertedPieceInfo(PreparedStatement statement, FileSymbol fileSymbol, RankSymbol rankSymbol,
                                      PieceSymbol pieceSymbol, ColorSymbol colorSymbol) {
        try {
    ...
    private void executeInitializeQuery(String query) {
