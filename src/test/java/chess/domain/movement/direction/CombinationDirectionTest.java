...
class CombinationDirectionTest {
    ...
    void Given_CombinationDirectionWithUpDirectionLeftDirection_When_CanReachWithReachablePosition_Then_True() {
        ...
        //when, then
        assertAll(
                () -> assertThat(combinationDirection.canReach(C1, C4, List.of())).isTrue(),
                () -> assertThat(combinationDirection.canReach(C1, A1, List.of())).isTrue()
        );
    }
    ...
    void Given_CombinationDirectionWithUpDirectionLeftDirection_When_CanReachWithUnreachablePosition_Then_False() {
        ...
        //when, then
        assertAll(
                () -> assertThat(combinationDirection.canReach(C4, C1, List.of())).isFalse(),
                () -> assertThat(combinationDirection.canReach(A1, C1, List.of())).isFalse()
        );
    }
}
